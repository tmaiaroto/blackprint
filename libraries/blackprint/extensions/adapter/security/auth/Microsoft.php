 ...
 */
class Microsoft extends \lithium\core\Object {

	/**
	 * Service name (lowercase, no special characters)
	 */
	protected $_service = 'microsoft';

	/**
	 * Full service name. Title casing, spaces, etc.
	*/
	protected $_serviceName = 'Microsoft';

	/**
	 * User infos URL
	 */
	protected $_userInfoEndpoint = 'https://apis.live.net/v5.0/me';
	...
	public function check($request, array $options = array()) {
		...
		if(empty($request->query['code'])) {
		    ...
		    if(is_array($credentials) && isset($credentials['id'])) {
		    	$result = array(
		    		'socialLogin' => array(
			    		...
			    		'name' => (!empty($credentials['name'])) ? $credentials['name']:trim($credentials['first_name'] . ' ' . $credentials['last_name']),
			    		...
			    		'userName' => null,
			    		'profilePicture' => 'https://apis.live.net/v5.0/' . $credentials['id'] . '/picture'
		    		)
		    	);
		    }
	    	...
	    	$result['socialLogin']['serviceIcon'] = '<i class="fa fa-windows"></i>';
	    	$result['socialLogin']['serviceUrl'] = 'http://www.microsoft.com';
