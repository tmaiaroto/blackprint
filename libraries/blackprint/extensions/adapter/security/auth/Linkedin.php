 ...
 */
class Linkedin extends \lithium\core\Object {

	/**
	 * Service name (lowercase, no special characters)
	 */
	protected $_service = 'linkedin';
	...
	*/
	 ...
	 * User infos URL
	 */
	protected $_userInfoEndpoint = 'people/~:(id,first-name,last-name,industry,picture-url,location:(country:(code)))?format=json';
	...
	public function __construct(array $config = array()) {
		$config += array('tokenKey' => 'oauth_token', 'scope' => array('r_fullprofile'));
	...
	public function check($request, array $options = array()) {
		...
		if(empty($request->query['code'])) {
		    $url = $service->getAuthorizationUri(array('state' => String::uuid()));
		    header('Location: ' . $url);
		} else {
		    ...
		    if(is_array($credentials) && isset($credentials['id'])) {
		    	$result = array(
		    		'socialLogin' => array(
			    		...
			    		'name' => trim($credentials['firstName'] . ' ' . $credentials['lastName']),
			    		...
			    		'locale' => null,
			    		'userName' => null,
			    		'profilePicture' => $credentials['pictureUrl']
		    		)
		    	);
		    }
	    	...
	    	$result['socialLogin']['serviceIcon'] = '<i class="fa fa-linkedin"></i>';
	    	$result['socialLogin']['serviceUrl'] = 'http://www.linkedin.com';
